...
def accuracy(network, loader, weights, device, args, pred_by = None):

    acc_dict = {}

    network.eval()
    with torch.no_grad():
        if args.algorithm in ['DoYoJo'] and pred_by is None and args.is_end:
            for key in ['alpha', 'beta', 'gamma']:
                acc_dict[key] = get_acc(network, loader, weights, device, args, pred_by = key)
        else:
            acc_dict['default'] = get_acc(network, loader, weights, device, args, pred_by=pred_by)
    network.train()

    return max(list(acc_dict.values()))

def get_acc(network, loader, weights, device, args, pred_by = None):
    correct = 0
    total = 0
    weights_offset = 0
    for x, y in loader:
        x = x.to(device)
        y = y.to(device)
        p = network.predict(x) if pred_by is None else network.predict(x, pred_by=pred_by)  # pred_by for DoYoJo
        if weights is None:
