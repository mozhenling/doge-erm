...
class ERM(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None, doyojo=None):
        ...
        all_logits = self.predict(all_x,  doyojo, is_all=True)
        # ------------------------------------------------------------------------
        nll = 0.
        all_logits_idx = 0
    ...
    def predict(self, x, doyojo=None, is_all = True):
        # ------------------------------------------------------------------------
        # ------------------------- DoYoJo sub-algorithm -------------------------
        if  doyojo is None:
            ...
            if is_all:
                return doyojo.subalg_all_logits(x)
            else:
                return doyojo.subalg_env_logits(x)
        # ------------------------------------------------------------------------
