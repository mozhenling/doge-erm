...
def print_table(table, header_text, row_labels, sub_alg_names_plus,col_labels, colwidth=10,
    latex=True):
    ...
    for row, label in zip(table, row_labels):
        if label in sub_alg_names_plus:
            if label[0] == 'DoYoJoAlpha':
                end_name = '_DYJA'
            else:
                end_name = '_DYJ'
            label_now = label[1] + end_name
        else:
            label_now =  label
        row.insert(0, label_now)
