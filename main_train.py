...
from datautils import data_process, bed_datasets as datasets_now, bed_dataloaders as dataloader
########################################################################################################################
#------------------------------------------------------------ Main
########################################################################################################################
...
if __name__ == "__main__":
    ...
    # where to save
    comp_alg = ['DoYoJo', 'DoYoJoAlpha']
    if args.sub_algorithm is None or args.sub_algorithm in [[], 'None']:
        folder_name = args.algorithm
    else:
        folder_name = args.algorithm+'_'+ args.sub_algorithm
    env_name = args.test_envs[0]
    args.output_dir = os.path.join(args.output_dir,
                                   args.dataset + '_test_id_' + str(env_name) + '_clean_label_' + str(
    ...
    # hparams['lr']=1e-2 #<---------------------------------------- manual weight tuning
    # hparams['recon_weight'] = 10
    # hparams['kld_weight'] = 1
    # hparams['erm_alpha_weight'] = 1
    # hparams['erm_beta_weight'] = 100
    # hparams['erm_gamma_weight'] = 1
    # hparams['sd_weight'] = 0 #1000000
    # --------------------------------------------------------------
    print('HParams:')
    ...
    """
    # print('Begin preparing the dataset...')
    if args.dataset in vars(datasets_now):
        # get an iterable of environment data
        dataset = vars(datasets_now)[args.dataset](args.data_dir, args.test_envs,
                                                   args.is_clean_label, hparams, args.label_flip_p)
    else:
    ...
    algorithm = algorithm_class(input_shape = dataset.input_shape,
                                ...
                                num_domains = len(dataset) - len(args.test_envs), # the number of training domains
                                hparams =hparams,
                                args=args)
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for name, loader, weights in evals:
                acc = optimization.accuracy(algorithm, loader, weights, device, args)
